...
def advancedDoubleBottom(closingPrices, openingPrices, highPrices, lowPrices):
    prices = closingPrices
    allBottoms = []
    lowestPoint = {"value": prices[0], "index": 0}
    highestPoint = {"value": prices[0], "index": 0}
    ...
    for i in range(len(prices)):
        ...
        if price > highestPoint['value']:
            highestPoint = {"value": price, "index": index}
    ...
    for i in range(len(allBottoms)):
        ...
        state = (
            ...
            and 1 > laggingBottom['value'] / midPoint['value'] >= midPointHeightThreshold 
...
def findHighestPointBetween(laggingBottom, leadingBottom, highPrices):
    subListLeftIndex = laggingBottom["index"] + 1
    subListRightIndex = leadingBottom["index"]
    subList = highPrices[subListLeftIndex:subListRightIndex]
    highestPoint = {"value": subList[0], "index": subListLeftIndex}
    for i in range(len(subList)):
        price = subList[i]
        if price > highestPoint['value']:
            highestPoint = {"value": price, "index": i + laggingBottom["index"]}
    return highestPoint
